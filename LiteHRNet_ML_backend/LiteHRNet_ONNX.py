...
ONNX_CHECKPOINT = 'Models/model_original_with_flip.onnx'


class LiteHRNet_ONNX_Model():
    def __init__(self, onnx_file_path=ONNX_CHECKPOINT) -> None:
    ...
    def pre_process(self, tensor_image) -> tuple:  # -> Any:
    ...
    def predict(self, img_path, task_id) -> tuple:
        local_img_path = get_local_path(img_path,
                                        access_token=LABEL_STUDIO_ACCESS_TOKEN,
                                        hostname=LABEL_STUDIO_HOST,
                                        task_id=task_id)
        original_img = io.read_image(local_img_path)
        pre_processed_image, _ = self.pre_process(original_img)
        heatmaps = self.sess.run(None,
                                 {self.net_feed_input[0]: pre_processed_image})[0]
        keypoints = self.post_process(heatmaps)
        return keypoints[0], pre_processed_image.shape[-2:], original_img.shape[-2:]
